...
from .util import _mandelbruh_GLOBAL_VARS, to_vec3, to_str
from . import camera
import os
...
class Shader:
	...
	def set(self, key, val):
		if key in _mandelbruh_GLOBAL_VARS:
			cur_val = _mandelbruh_GLOBAL_VARS[key]
			if type(val) is float and type(cur_val) is not float:
				val = to_vec3(val)
		_mandelbruh_GLOBAL_VARS[key] = val
		if key in self.keys:
	...
	def get(self, key):
		if key in _mandelbruh_GLOBAL_VARS:
			return _mandelbruh_GLOBAL_VARS[key]
		return None
	...
	def compile(self, cam):
		...
		var_code = ''
		for k in _mandelbruh_GLOBAL_VARS:
			typ = 'float' if type(_mandelbruh_GLOBAL_VARS[k]) is float else 'vec3'
			var_code += 'uniform ' + typ + ' _' + k + ';\n'

		split_ix = f_shader.index('// [/pyvars]')
		f_shader = f_shader[:split_ix] + var_code + f_shader[split_ix:]

		#Create code for all mandelbruh
		nested_refs = {}
		...
		split_ix = f_shader.index('// [/mandelbruh]')
